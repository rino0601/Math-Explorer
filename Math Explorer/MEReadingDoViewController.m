...
-(void)viewDidLoad {
	...
	NSError *err=nil;
	playerProblem=[[AVAudioPlayer alloc] initWithContentsOfURL:[[[NSBundle mainBundle] resourceURL] URLByAppendingPathComponent:[NSString stringWithFormat:@"me.problem.%d.%03d.m4a", langCode, (langCode==2?problemID-288:problemID)]] error:&err];
	[playerProblem setVolume:1.0f];
	[playerProblem prepareToPlay];
	playerTitle=[[AVAudioPlayer alloc] initWithContentsOfURL:[[[NSBundle mainBundle] resourceURL] URLByAppendingPathComponent:[NSString stringWithFormat:@"me.reading.do.instruction.0.%d.m4a",langCode]] error:&err];
	...
	sqlite3_step(localizer);
	[meReadingDoInstruction setTitle:[NSString stringWithUTF8String:(const char *)sqlite3_column_text(localizer, 0)] forState:UIControlStateNormal];
...
-(void)sayButtonAction:(id)sender {
	if([playerProblem isPlaying]==NO) {
		[playerProblem setCurrentTime:0.0];
		[playerProblem play];
	} else
		[playerProblem stop];
}
